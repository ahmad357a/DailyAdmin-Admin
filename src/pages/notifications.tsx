...
const NotificationsPage = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const apiUrl = (import.meta.env.VITE_API_URL || 'https://daily-earn-backend-production.up.railway.app').replace(/\/+$/, '');
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      await axios.put(
        `${(import.meta.env.VITE_API_URL || 'https://daily-earn-backend-production.up.railway.app').replace(/\/+$/, '')}/api/user/notifications/${notificationId}/read`,
        {},
        { 
          withCredentials: true,
          timeout: 10000
  ...
  const markAllAsRead = async () => {
    try {
      await axios.put(
        `${(import.meta.env.VITE_API_URL || 'https://daily-earn-backend-production.up.railway.app').replace(/\/+$/, '')}/api/user/notifications/read-all`,
        {},
        { 
          withCredentials: true,
          timeout: 10000
